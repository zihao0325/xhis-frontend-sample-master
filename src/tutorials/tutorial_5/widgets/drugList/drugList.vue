...
export default {
  ...
  setup() {
    ...
    const handler = new Map<string, (resultData) => void>();
    handler['OpenUrl'] = (resultData) => {
      const url = resultData.result.url;
      window.open(url);
    };
    // custom action
    // #region OpenSearch
    handler['OpenSearch'] = (resultData) => {
      const keyword = resultData.result.keyword;
      window.open(`https://google.com/search?q=${keyword}`);
    };
    // #endregion OpenSearch
    handler['ShowDialog'] = (resultData) => {
      dialogProps.value = resultData.result;
      showDialogue.value = true;
    };
    handler['NoResult'] = (resultData) => {
