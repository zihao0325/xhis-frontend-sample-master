...
import engineSet, { DrugSchema } from '@asus-aics/x-fe-engine';
import { api } from '@/utils/api';
import { PrescribeMode, TriggerTypes } from '@asus-aics/xhis-rule-schema';
...
const updateItem = (
  ...
  const prev = item?.[col]?.previous;
  const engineColumn = engineData.value.find((data) => data.itemId.value === item.itemId.value);
  const runEditRule = async (drug, col) => {
    const resp = await api.runRule({
      encounterSyncId: encounterEngine.core.getSyncId(),
      opdSyncId: engine.core.getSyncId(),
      triggers: [
        {
          type: TriggerTypes.DrugValueChanged,
          payload: [`drugList[itemId.value=${drug.itemId.value}].${col}.value`],
  ...
  if (storedValue !== prev) {
    updateColumn(engineColumn, col, storedValue);

    runEditRule(item, col);
  }
};

export const useEditComponent = (encounterDrugList: any) => {
  ...
  const handleCellBlur = (item: EncounterDrugSchema, index: string) => {
    if (columnTmpValue[getColumnKey(item, index)] !== undefined) {
      updateItem(item, index, columnTmpValue[getColumnKey(item, index)], encounterDrugList);
    }
