    ...
    >
      ...
      <template #cell-actions="{ item }">
        <XButton
          ...
          @click="() => handleRowClick(item, 'icd')"
          >ICD</XButton
        >
        <XButton icon="drug-colored" size="sm" outline @click="() => handleRowClick(item, 'drug')">Drug</XButton>
      </template>
    </XTable>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const { engine: encounterEngine } = engineSet.useEncounterStateEngine();
    ...
    const handleRowClick = async (item: any, dest: string) => {
      await encounterEngine.switch(item.encounterId, item);
      sendEngineEvent(dest);
    };
